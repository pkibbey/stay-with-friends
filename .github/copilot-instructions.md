...
- This is a Turborepo monorepo with two main apps:
  ...
  - `apps/backend` — TypeScript Express server (port 4000 in dev). See `apps/backend/src/index.ts`, and `apps/backend/src/db.ts`.
...
5) Things to watch for when editing code
...
- Prefer using the central DB exported in `db.ts` for predictable lifecycle.

6) Useful examples to copy/pattern-match
...
8) Pragmatic editing rules for AI
...
- When adding DB schema fields, also update `db.ts` migrations and any affected prepared statements.
- When modifying types, prefer editing `@stay-with-friends/shared-types` and importing types rather than generating new ones. See below for details.
